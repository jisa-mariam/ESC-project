/*
FAULT BASED TEST CASE 2

Input project has attribute "assigned" set to be true

For test to pass:
Project should not be drawn
*/

function testGeneralAllocationDrawn2(myDoc){
    var arrayBoundary = [];
    var arrayCapstone = [];
    ...
    // millimeter units are obtained by multiplying the values by 0.3528
...
function main(){
    ...
    testGeneralAllocationDrawn2(banana);
}
...
function checkDrawnBoxes(myDoc){
    ...
    for(var k = 0; k < selectionLength; k++){
        ...
        if(bHeight == 10 && bWidth == 20){
            var newWin = new Window("dialog", 'Box drawn');
            newWin.add("statictext", undefined, 'Algorithm drew a pre-assigned proejct, function has failed the test');
            newWin.show();
            return false;
            }
        }
    var newWin = new Window("dialog", 'Box not drawn');
    newWin.add("statictext", undefined, 'Algorithm did not draw a pre-assigned project, function passed test');
    newWin.show();
    }
